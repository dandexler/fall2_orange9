# Team Orange 2: Cathy Tran, Zach Lewis, David Andexler, Kyrsten Rudock, Savannah Hampton
# Ocotober 08, 2019
# HW 4 Time Series 
...
Pacf(quad.res, lag=48, main = "PACF No MA or AR terms")$acf 

# Dummy variable and Quadratic models that I will try
# Model 1: ARIMA(1,0,0)(0,0,1)
...
# Yes, white noise. Beautiful! - quad, seasonality
...
# Won't use as the final model on validation data
...
rar1.rma1.sar1.sma1.q= Arima(ts.months.train, xreg=sq_new_xreg,order=c(1,0,1), seasonal=c(1,0,1))
summary(rar1.rma1.sar1.sma1.q)

Acf(rar1.rma1.sar1.sma1.q$residuals, lag=36, main='ACF (1,0,1)(1,0,1) ')$acf
Pacf( rar1.rma1.sar1.sma1.q$residuals, lag=36, main='PACF(1,0,1)(1,0,1)')$acf
...
White.LB <- rep(NA, 24)
for(i in 1:24){
  White.LB[i] <- Box.test(rar1.rma1.sar1.sma1.q$residuals, lag = i, type = "Ljung", fitdf = 4)$p.value
}
...
# White noise achieved but not as obvious and evident as ARIMA(1,0,0)(0,0,1)
...
rar1.rma1.q = Arima(ts.months.train, xreg=sq_new_xreg,order=c(1,0,1), seasonal=c(0,0,0))
summary(rar1.rma1.q) 


Acf(rar1.rma1.q$residuals, lag=36,main = " ACF (1,0,1)(0,0,0) ")$acf
Pacf(rar1.rma1.q$residuals, lag=36, main = "PACF (1,0,1)(0,0,0) ")$acf
...
rma1.sar1.q= Arima(ts.months.train, xreg=sq_new_xreg,order=c(0,0,1), seasonal=c(1,0,0))
summary(rma1.sar1.q)

Acf(rma1.sar1.q$residuals, lag=36, main='ACF (0,0,1)(1,0,0)  ')$acf
Pacf( rma1.sar1.q$residuals, lag=36, main='PACF (0,0,1)(1,0,0) ')$acf

# Maybe bc ACF lag 24 is very close to the confidence limit
# Won't use on validation data to honor the fact that we can't cheat by testing our training model on validation data multiple time
# High AIC and BIC values (goodness-of-fit)
...
rma1.sma1.q= Arima(ts.months.train, xreg=sq_new_xreg,order=c(0,0,1), seasonal=c(0,0,1))

Acf(rma1.sma1.q$residuals, lag=36, main='ACF (0,0,1)(0,0,1) ')$acf
Pacf( rma1.sma1.q$residuals, lag=36, main='PACF (0,0,1)(0,0,1)')$acf
...
White.LB <- rep(NA, 24)
for(i in 1:24){
  White.LB[i] <- Box.test(rma1.sma1.q$residuals, lag = i, type = "Ljung", fitdf = 2)$p.value
}
...
# NO!!! No, still significant correlation left
...
Pacf(quad.forecast$residuals, lag=36, main = "PACF (1,0,0)(0,0,1) ")$acf
...
# Model 3 ARIMA(1,0,1)(1,0,1) Forecast
rar1.rma1.sar1.sma1.q.forecast = forecast(rar1.rma1.sar1.sma1.q, xreg=nreg_qmatrix,h = 6)
summary(rar1.rma1.sar1.sma1.q.forecast)

# Plot the forecast
plot(rar1.rma1.sar1.sma1.q.forecast)

actual = unclass(ts.months.valid)
pred = unclass(rar1.rma1.sar1.sma1.q.forecast$mean)
...
# Model 6 ARIMA(0,0,1)(0,0,1) Forecast
rma1.sma1.q.forecast = forecast(rma1.sma1.q, xreg=nreg_qmatrix,h = 6)
summary(rma1.sma1.q.forecast)

# Plot the forecast
plot(rma1.sma1.q.forecast)

actual = unclass(ts.months.valid)
pred = unclass(rma1.sma1.q.forecast$mean)
...
# Set my new f.xreg so the Arima model can regress on both the fourier model and the linear trend component
...
#######################################
# 
# Wrote the code for Fourrier forecasting but didn't forecast on 
# validation data because this model was built on fourier functions and linear trend
# Quadratic trend better fit the model
# Season length = 12 not long and complex so dummy variable fit better
#######################################
